<?php 	
	...
	class FNDownlineLevelModel extends Base_model
	{
			...
			{	
				...
				{	
					...
					{	
						if($value2->L_R == "LEFT")
						{
							$this->db->query(" SELECT COUNT(*) as number FROM `users` WHERE upline = '$value2->id' and status != 'pre-activated'  ");
				...
				{	
							...
							{	
								foreach ($left_result as $key3 => $value3) 
								{
									...
									if($value3->L_R == "LEFT")
									{
										...
										$this->db->query(" SELECT id FROM `users` WHERE upline = '$value3->id' and status != 'pre-activated'  ");
						...
						}else 
						{
							...
							{	
								...
								{	
									if($value4->L_R == "LEFT")
									{
										$this->db->query(" SELECT id FROM `users` WHERE upline = '$value4->id' and status != 'pre-activated'  ");
					...
					{	
						...
						for($row = 0; $row < 8; $row++)
						{
							...
							$this->db->query(" SELECT COUNT(*) as number FROM `users` WHERE upline = '$ids[$row]' and status != 'pre-activated' ");
			...
			{	
				...
				{	
					...
					{	
						...
						for($row = 0; $row < 8; $row++)
						{
							$this->db->query(" SELECT id FROM `users` WHERE upline = '$ids[$row]' and status != 'pre-activated'  ");
						...
						{	
							...
							{	
								...
								$this->db->query(" SELECT COUNT(*) as number FROM `users` WHERE upline = '$ids2[$row]' and status != 'pre-activated' ");
					    	 	array_push($downline_counter, $this->db->single()->number );
