        ...
        <div class="card">
            ...
            <div class="card-body">
                <?php
                    ...
                    Form::hidden('userid', $uMeta->userid);
                    Form::hidden('id', $uMeta->id);
                ?>

                <div class="form-group">
                    <label for="#">Category</label>
                    <select name="meta_key" id="" class="form-control">
                        <optgroup label="Income">
                            <?php foreach($this->userService::incomeKeys() as $key => $row) :?>
                                <?php $isSelected = $uMeta->meta_key == $row ? 'selected' : ''?>
                                <option value="<?php echo $row?>" <?php echo $isSelected?>><?php echo $row?></option>
                            <?php endforeach?>
                        </optgroup>
                    ...
                    <div class="form-group">
                        <?php
                            Form::label('Answer');
                            if (strlen($uMeta->meta_value) > 100) {
                                Form::textarea('meta_value', $uMeta->meta_value, [
                                    'class' => 'form-control',
                                    'required' => true,
                                    'rows' => 3
                                ]);
                            } else {
                                Form::text('meta_value', $uMeta->meta_value, [
                                    'class' => 'form-control',
                                    'required' => true
                                ]);
                            }
                    ...
                    <div class="form-group">
                        <?php
                            ...
                            Form::textarea('meta_attribute[description]', $uMeta->meta_attribute->description ?? '', [
                                'class' => 'form-control'
                            ]);
                        ?>
                    </div>
                    ...
                    <div class="form-group" id="bankName">
                        <?php
                            ...
                            Form::select('meta_attribute[bank_org]', $bankNames ,$uMeta->meta_attribute->bank_org ?? '', [
                                'class' => 'form-control',
                                'id' => 'bankNameField'
                            ]);
                        ?>
                    ...
                    <div class="form-group">
                        ...
                        <a href="/Usermeta/delete/<?php echo $uMeta->id?>?returnTo=<?php echo seal("/FinancialStatementController")?>" class="btn btn-danger">Delete</a>
                    </div>
                <?php Form::close()?>
            </div>
        </div>
...
<?php build('scripts')?>
    <script>
        $(document).ready(function() {
            $("#bankNameOther").hide();
            <?php 
                if(!isEqual($uMeta->meta_key,'bank')) {
                    ?> 
                        $("#bankName").hide();
                    <?php
                }
